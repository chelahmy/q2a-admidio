...
function qa_get_login_links($relative_url_prefix, $redirect_back_to_url)
{
	...
	*/
	...
	return array(
		'login' => 'https://agromaya.net/adm_program/system/login.php?redirect=' . urlencode('https://ask.agromaya.net/' . $redirect_back_to_url),
		'register' => 'https://agromaya.net/adm_program/modules/registration/registration.php?redirect=' . urlencode('https://ask.agromaya.net/' . $redirect_back_to_url),
		'logout' => 'https://agromaya.net/adm_program/system/logout.php?redirect=' . urlencode('https://ask.agromaya.net/' . $redirect_back_to_url)
	);
}
...
function qa_get_logged_in_user()
{
	...
	*/
    ...
    $sid = get_admidio_session_id();

    if ($sid !== false) {
        $uid = get_admidio_user_id_from_session_id($sid);

        if ($uid > 0) {
            $uname = get_admidio_user_name($uid);
            
            if ($uname !== false && strlen($uname) > 0) {
                $email = get_admidio_user_email($uid);
...
function qa_get_public_from_userids($userids)
{
	...
	*/
    ...
    return get_admidio_user_names($userids);
}
...
function qa_avatar_html_from_userid($userid, $size, $padding)
{
	...
	*/
	...
	return '<img src="qa-external/q2a-admidio-avatar.php?usr_id=' . htmlspecialchars($userid) . '&size=' . $htmlsize . '" ' .
		'width="' . $htmlsize . '" height="' . $htmlsize . '" class="qa-avatar-image" alt=""/>';
}
